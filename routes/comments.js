...
router.put('/campgrounds/:id/comments/:comments_id', middleware.checkCommentOwnership, function(
    req,
    res
) {
...
router.delete('/campgrounds/:id/comments/:comments_id', middleware.checkCommentOwnership, function(req,res) {
    Comment.findByIdAndRemove(req.params.comments_id, function(err, RemovedComment) {
        if (err) {
            req.flash('error', 'Something went wrong');
            res.redirect('back');
        } else {
